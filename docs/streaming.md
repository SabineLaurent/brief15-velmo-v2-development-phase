...
> Note de référence. Quand le streaming token par token a été branché, une
> question a surgi : **qui**, en prod, « gère » le streaming —
...
concaténation — exactement ce que faisait le client avant qu'on ajoute
`stream_token`.
...
branche support) ; on le traite en montrant les **étapes**, pas en
...
Le **garde de sortie** (`guard_output`) est un **nœud postérieur**
...
la FAQ… »). Le contrat reste un générateur async : y ajouter des étapes revient
à yielder plus de chunks, **sans modifier aucun front**.
...
- Le flux ne contient toujours **qu'un chunk** (la réponse gardée), mais chaque
  couche le traite **comme un flux** : le jour où la couture en yield plusieurs, rien
  à reprendre. Le transport SSE est déjà **typé** (`{"type": "chunk"|"error"|"done"}`)
...
- **Plus tard, avec un front React.** Rien de nouveau à construire côté agent :
