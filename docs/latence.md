...
> Note de référence. En reprenant le front, une question a surgi : **pourquoi ~5 secondes de silence avant que l'agent ne
...
> valent le coup **ici**, et lesquels sont « prod-grade » à documenter
> comme cible sans les coder maintenant.
> Complète [`streaming.md`](streaming.md) (§ « le silence avant le 1er token »)
> et [`architecture.md`](architecture.md) (le graphe).
...
1. **Moins de hops séquentiels.** On fait **3 allers-retours LLM** avant de
   ...
   est un choix de lisibilité assumé, cf. [`architecture.md`](architecture.md)).
...
## Ce qui est réaliste **chez nous** — l'ordre
...
2. **Prompt caching Azure** sur le dernier appel. Levier n°1 de la prod, faible
   risque. Gain **~0.5–1 s**.
3. **Steps / feedback côté front** : le
...
1. Sur cette infra Azure, **le TTFT est dominé par le coût par hop**
...
Le seul gros levier restant : **réduire le nombre de hops**
(supprimer ou fusionner le routeur ≈ un hop de moins ≈ ~1.2 s, mais ça casse le
« capot ouvert » qu'on montre exprès).

## Ce qu'on laisse hors périmètre

Leviers identifiés comme cible prod-grade, **pas** à coder maintenant (mises en
place détaillées, avec code illustratif, dans
[`latence-patterns-prod.md`](latence-patterns-prod.md)) :
...
> steps front**. Le reste est une cible prod-grade, à ne pas coder maintenant.
