...
> Note de référence (cible **prod-grade**, pas du code à écrire maintenant). `docs/latence.md`
...
préservé : les embeddings passent par la factory, le cache par un
...
## Ce que ça coûte (et pourquoi c'est une cible, pas du code d'aujourd'hui)
...
à adopter **un levier à la fois** si on décide un jour de dépasser la démo.
