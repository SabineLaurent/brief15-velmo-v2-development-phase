...
def _tickets() -> list[Ticket]:
    ...
    rows = [
        (
            "C-marc-dubois",
            "Colis marqué livré mais non reçu (O-2024-0105)",
            "Le suivi indiquait 'livré' mais le client n'a rien reçu.",
            "resolved",
        ),
        (
            "C-sophie-martin",
            "Doute sur l'authenticité du maillot Brésil 1970",
            "La cliente demande le certificat d'authenticité de la commande O-2024-0110.",
            "open",
        ),
    ]
    return [
        Ticket(
            id=_ticket_id(customer_id, subject, body),
        ...
        for customer_id, subject, body, status in rows
    ]


def seed(session) -> None:
    """Insert the reference dataset into the given session.

    Batches are flushed in order because the foreign keys demand it: a variant
    needs its product, an order item needs both its order and its variant.
    """
    for batch in (
        _customers(),
        _products(),
